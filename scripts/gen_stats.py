...
for i, (label, val, note, color) in enumerate(STATS):
    ...
    rows.append(f'''<g>
    <circle cx="{x+6}" cy="{y-14}" r="3.5" fill="{color}"/>
...
scan_distance = max((len(top) - 1) * 26, 0)
svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" role="img" aria-label="Live GitHub statistics and top languages" font-family="'JetBrains Mono', ui-monospace, SFMono-Regular, monospace">
  ...
  {"".join(rows)}
  {"".join(bars)}
  <g class="motion" opacity="0.85">
    <rect x="{bx+92}" y="76" width="2" height="15" rx="1" fill="#dff6ff">
      <animateTransform attributeName="transform" type="translate" values="0 0;0 {scan_distance};0 {scan_distance};0 0" keyTimes="0;0.72;0.86;1" dur="5.6s" repeatCount="indefinite"/>
    </rect>
    <path d="M{bx+88} 76H{bx+102}" stroke="#36bcf7" stroke-width="1">
      <animateTransform attributeName="transform" type="translate" values="0 0;0 {scan_distance};0 {scan_distance};0 0" keyTimes="0;0.72;0.86;1" dur="5.6s" repeatCount="indefinite"/>
    </path>
  </g>
