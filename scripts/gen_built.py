...
def verification_glyph(slug, accent, cx, cy):
    """Project-specific motion mark. Static geometry remains under reduced motion."""
    if slug == "proof-of-work":
        return f'''<g aria-label="signature verification">
    <path d="M{cx-14} {cy+5}L{cx-6} {cy+12}L{cx+15} {cy-12}" fill="none" stroke="{accent}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    <path d="M{cx-16} {cy-14}H{cx+16}" stroke="{accent}" stroke-opacity="0.28"/>
    <line class="motion" x1="{cx-16}" y1="{cy-14}" x2="{cx-7}" y2="{cy-14}" stroke="#f6c69e" stroke-width="2" stroke-linecap="round">
      <animateTransform attributeName="transform" type="translate" values="0 0;23 0;0 0" keyTimes="0;0.72;1" dur="3.8s" repeatCount="indefinite"/>
    </line></g>'''
    if slug == "gfi-scout":
        return f'''<g aria-label="issue radar">
    <circle cx="{cx}" cy="{cy}" r="14" fill="none" stroke="{accent}" stroke-opacity="0.35"/>
    <circle cx="{cx}" cy="{cy}" r="7" fill="none" stroke="{accent}" stroke-opacity="0.22"/>
    <circle cx="{cx+7}" cy="{cy-6}" r="2.5" fill="{accent}"/>
    <line class="motion" x1="{cx}" y1="{cy}" x2="{cx}" y2="{cy-13}" stroke="{accent}" stroke-width="1.5" stroke-linecap="round">
      <animateTransform attributeName="transform" type="rotate" from="0 {cx} {cy}" to="360 {cx} {cy}" dur="4.6s" repeatCount="indefinite"/>
    </line></g>'''
    if slug == "obsidian":
        return f'''<g aria-label="knowledge graph">
    <path d="M{cx-13} {cy+8}L{cx-4} {cy-11}L{cx+14} {cy+5}Z" fill="none" stroke="{accent}" stroke-opacity="0.38"/>
    <circle cx="{cx-13}" cy="{cy+8}" r="3" fill="{accent}"/><circle cx="{cx-4}" cy="{cy-11}" r="3" fill="{accent}"/><circle cx="{cx+14}" cy="{cy+5}" r="3" fill="{accent}"/>
    ...
    <rect x="{cx-14}" y="{cy+3}" width="11" height="11" rx="2" fill="{accent}" fill-opacity="0.12" stroke="{accent}"/>
    <rect x="{cx+3}" y="{cy+3}" width="11" height="11" rx="2" fill="{accent}" fill-opacity="0.22" stroke="{accent}"/>
    <path class="motion" d="M{cx-9} {cy+8}L{cx-3} {cy+14}L{cx+15} {cy-9}" fill="none" stroke="#a7f3b4" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      <animate attributeName="stroke-dasharray" values="0 48;48 0;48 0" keyTimes="0;0.55;1" dur="4s" repeatCount="indefinite"/>
    </path></g>'''
...
for slug, mono, accent, url, starrepo, title, lines, tags, note in CARDS:
    ...
    svg = (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" '
           f'role="img" aria-label="{esc(title)} project summary" font-family="\'JetBrains Mono\', ui-monospace, SFMono-Regular, monospace">\n  '
           f'<style>.motion {{ visibility: hidden; }} @media (prefers-reduced-motion: no-preference) {{ .motion {{ visibility: visible; }} }}</style>\n  '
           + "\n  ".join(parts) + "\n</svg>\n")
